...
def crnn():
    ...
    num_classes=36+1
    x=keras.layers.Dense(num_classes, name='outputdense')(x)#알파벳+숫자#1953 36
    
    x=y_pred= Activation('softmax', name='outputsoftmax')(x)#1953 36
...
# In[6]
traincrnn.compile(optimizer=tf.keras.optimizers.Adam(lr=0.001, epsilon=0.001,decay=1e-5, clipnorm=1.) ,loss={'ctcloss':lambda y_true,y_pred:y_pred})
